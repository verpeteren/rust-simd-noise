...
unsafe fn round_sse2(a: __m128) -> __m128 {
    ...
    let vNearest2 = _mm_castsi128_ps(_mm_srli_epi32(_mm_castps_si128(v1), 2));
    let i = _mm_cvttps_epi32(a);
    let aTrunc = _mm_cvtepi32_ps(i); // truncate a
    let rmd = _mm_sub_ps(a, aTrunc); // get remainder
    let rmd2 = _mm_mul_ps(rmd, vNearest2); // mul remainder by near 2 will yield the needed offset
    let rmd2i = _mm_cvttps_epi32(rmd2); // after being truncated of course
    let rmd2Trunc = _mm_cvtepi32_ps(rmd2i);
    _mm_add_ps(aTrunc, rmd2Trunc)
}
