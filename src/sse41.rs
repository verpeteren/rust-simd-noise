...
unsafe fn hash_1d(seed: __m128i, x: __m128i) -> __m128i {
...
#[target_feature(enable = "sse4.1")]
unsafe fn val_coord_1d(seed: __m128i, x: __m128i) -> __m128 {
    let mut hash = _mm_xor_si128(seed, _mm_mullo_epi32(X_PRIME, x));
    hash = _mm_mullo_epi32(
        hash,
        _mm_mullo_epi32(hash, _mm_mullo_epi32(hash, _mm_set1_epi32(60493))),
...
pub unsafe fn cellular_1d(
    ...
    match distance_function {
        CellDistanceFunction::Euclidean => {
            for xmod in -1..2 {
                ...
                let hi = M128iArray {
                    simd: _mm_and_si128(hash_1d(_mm_set1_epi32(1337), xi), _mm_set1_epi32(0xff)),
                };
                let cellx = M128Array {
                    array: [
                ...
                let vx = _mm_add_ps(
                    _mm_sub_ps(_mm_cvtepi32_ps(xi), x),
                    _mm_mul_ps(cellx.simd, jitter),
                );

                let new_dist = _mm_mul_ps(vx, vx);
        ...
        CellDistanceFunction::Natural => {
            for xmod in -1..2 {
                ...
                let new_dist = _mm_add_ps(_mm_abs_ps(vx), _mm_mul_ps(vx, vx));
                let cond = _mm_cmplt_ps(new_dist, distance);
                distance = _mm_blendv_ps(distance, new_dist, cond);
                xc = blendvi_sse2(xc, xi, _mm_castps_si128(cond));
            }
    ...
    match return_type {
        ...
        CellReturnType::CellValue => val_coord_1d(_mm_set1_epi32(1337), xc),
    }
}
#[target_feature(enable = "sse4.1")]
        ...
        CellDistanceFunction::Manhattan => {
            for xmod in -1..2 {
                ...
                for ymod in -1..2 {
                    ...
                    let new_dist = _mm_add_ps(_mm_abs_ps(vx), _mm_abs_ps(vy));
                    let cond = _mm_cmplt_ps(new_dist, distance);
                    distance = _mm_blendv_ps(distance, new_dist, cond);
                    xc = blendvi_sse2(xc, xi, _mm_castps_si128(cond));
                    yc = blendvi_sse2(yc, yi, _mm_castps_si128(cond));
...
unsafe fn hash_3d(seed: __m128i, x: __m128i, y: __m128i, z: __m128i) -> __m128i {
    let mut hash = _mm_xor_si128(seed, _mm_mullo_epi32(X_PRIME, x));
    hash = _mm_xor_si128(hash, _mm_mullo_epi32(Y_PRIME, y));
    hash = _mm_xor_si128(hash, _mm_mullo_epi32(Z_PRIME, z));
    hash = _mm_mullo_epi32(
        hash,
        _mm_mullo_epi32(hash, _mm_mullo_epi32(hash, _mm_set1_epi32(60493))),
    );
...
pub unsafe fn cellular_3d(
    ...
    let zr = _mm_cvtps_epi32(_mm_round_ps(
        z,
        _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC,
    ));
    let mut distance = _mm_set1_ps(f32::MAX);
...
unsafe fn grad1_simd(hash: __m128i, x: __m128) -> __m128 {
    ...
    let grad = _mm_blendv_ps(_mm_sub_ps(_mm_setzero_ps(), v), v, h_and_8);
    _mm_mul_ps(grad, x)
}
...
unsafe fn grad4_simd(hash: __m128i, x: __m128, y: __m128, z: __m128, t: __m128) -> __m128 {
    ...
    let v = _mm_blendv_ps(z, y, mask);
    mask = _mm_castsi128_ps(_mm_cmplt_epi32(h, _mm_set1_epi32(8)));
    let w = _mm_blendv_ps(t, z, mask);
    ...
    _mm_add_ps(
        ...
        _mm_add_ps(
            _mm_blendv_ps(_mm_sub_ps(_mm_setzero_ps(), v), v, h_and_2),
            _mm_blendv_ps(_mm_sub_ps(_mm_setzero_ps(), w), w, h_and_4),
        ),
    )
}
...
pub unsafe fn simplex_4d(x: __m128, y: __m128, z: __m128, w: __m128) -> __m128 {
    ...
    let jps = _mm_floor_ps(_mm_add_ps(y, s));
    let kps = _mm_floor_ps(_mm_add_ps(z, s));
    let lps = _mm_floor_ps(_mm_add_ps(w, s));
