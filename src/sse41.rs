...
{	
	...
	for _ in 1..octaves 
	{
		...
		yf = _mm_mul_ps(yf, lac_s);		
		amp = _mm_mul_ps(amp, gain_s);
		result = _mm_add_ps(result,_mm_abs_ps(_mm_mul_ps(simplex_2d_sse41(xf, yf),amp)));
	}
...
unsafe fn simplex_3d_sse41(x:__m128, y:__m128,z:__m128) -> __m128 {
    ...
    let mut i = M128iArray{ simd: _mm_cvtps_epi32(_mm_floor_ps(_mm_add_ps(x,s.simd)))};
    let mut j = M128iArray{ simd: _mm_cvtps_epi32(_mm_floor_ps(_mm_add_ps(y,s.simd)))};
    let mut k = M128iArray{ simd: _mm_cvtps_epi32(_mm_floor_ps(_mm_add_ps(z,s.simd)))};
