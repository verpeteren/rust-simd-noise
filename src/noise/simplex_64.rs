...
use crate::noise::cellular::{X_PRIME_64, Y_PRIME_64, Z_PRIME_64};
use crate::noise::gradient_64::{grad1, grad2, grad3d, grad3d_dot, grad4};
...
pub unsafe fn simplex_1d_deriv<S: Simd>(x: S::Vf64, seed: i64) -> (S::Vf64, S::Vf64) {
    ...
    i0 = i0 & 0xff;
    let gi0 = S::i64gather_epi64(&PERM, i0);
    let gi1 = S::i64gather_epi64(&PERM, i1);
    ...
    let value = (n0 + n1) * SCALE;
    let derivative =
        ((t20 * t0 * gx0 * x20 + t21 * t1 * gx1 * x21) * -8.0 + t40 * gx0 + t41 * gx1) * SCALE;
    (value, derivative)
}
...
pub unsafe fn simplex_2d_deriv<S: Simd>(
    ...
    let x1 = x0 + i1.cast_f64() + G2_64;
    let y1 = y0 + j1.cast_f64() + G2_64;
    let x2 = x0 + (-1.0) + G22_64;
    let y2 = y0 + (-1.0) + G22_64;

    let ii = i & 0xff;
    let jj = j & 0xff;

    let gi0 = S::i64gather_epi64(&PERM, ii + S::i64gather_epi64(&PERM, jj));

    let gi1 = S::i64gather_epi64(&PERM, (ii - i1) + S::i64gather_epi64(&PERM, jj - j1));

    let gi2 = S::i64gather_epi64(&PERM, (ii + 1) + S::i64gather_epi64(&PERM, jj + 1));

    // Weights associated with the gradients at each corner
    // These FMA operations are equivalent to: let t = 0.5 - x*x - y*y
    let mut t0 = y0.neg_mul_add(y0, x0.neg_mul_add(x0, S::Vf64::set1(0.5)));
    let mut t1 = y1.neg_mul_add(y1, x1.neg_mul_add(x1, S::Vf64::set1(0.5)));
    let mut t2 = y2.neg_mul_add(y2, x2.neg_mul_add(x2, S::Vf64::set1(0.5)));
    ...
    let scale = 45.26450774985561631259;
...
pub unsafe fn simplex_3d_deriv<S: Simd>(
    ...
    let i = x0.cast_i64() * X_PRIME_64;
    let j = y0.cast_i64() * Y_PRIME_64;
    let k = z0.cast_i64() * Z_PRIME_64;

    // Compute distance from first simplex vertex to input coordinates
    let g = (x0 + y0 + z0) * G3_64;
    x0 = x - (x0 - g);
    y0 = y - (y0 - g);
    z0 = z - (z0 - g);
    ...
    let x1 = x0 - (i1 & 1.0) + G3_64;
    let y1 = y0 - (j1 & 1.0) + G3_64;
    let z1 = z0 - (k1 & 1.0) + G3_64;

    let x2 = x0 - (i2 & 1.0) + F3_64;
    let y2 = y0 - (j2 & 1.0) + F3_64;
    let z2 = z0 - (k2 & 1.0) + F3_64;
    ...
    let v1x = i + (i1.cast_i64() & X_PRIME_64);
    let v1y = j + (j1.cast_i64() & Y_PRIME_64);
    let v1z = k + (k1.cast_i64() & Z_PRIME_64);
    let g1 = grad3d_dot::<S>(seed, v1x, v1y, v1z, x1, y1, z1);
    let v1 = t41 * g1;

    let v2x = i + (i2.cast_i64() & X_PRIME_64);
    let v2y = j + (j2.cast_i64() & Y_PRIME_64);
    let v2z = k + (k2.cast_i64() & Z_PRIME_64);
    ...
    let v3x = i + X_PRIME_64;
    let v3y = j + Y_PRIME_64;
    let v3z = k + Z_PRIME_64;
...
pub unsafe fn simplex_4d<S: Simd>(
    ...
    rank_x = rank_x + (cond & 1);
    rank_y = rank_y + (!cond & 1);
    let cond = x0.cmp_gt(z0).cast_i64();
    rank_x = rank_x + (cond & 1);
    rank_z = rank_z + (!cond & 1);
    let cond = x0.cmp_gt(w0).cast_i64();
    rank_x = rank_x + (cond & 1);
    rank_w = rank_w + (!cond & 1);
    ...
    let i1 = rank_x.cmp_gt(S::Vi64::set1(2)) & 1;
    let j1 = rank_y.cmp_gt(S::Vi64::set1(2)) & 1;
    let k1 = rank_z.cmp_gt(S::Vi64::set1(2)) & 1;
    let l1 = rank_w.cmp_gt(S::Vi64::set1(2)) & 1;

    let i2 = rank_x.cmp_gt(S::Vi64::set1(1)) & 1;
    let j2 = rank_y.cmp_gt(S::Vi64::set1(1)) & 1;
    let k2 = rank_z.cmp_gt(S::Vi64::set1(1)) & 1;
    let l2 = rank_w.cmp_gt(S::Vi64::set1(1)) & 1;

    let i3 = rank_x.cmp_gt(S::Vi64::zeroes()) & 1;
    let j3 = rank_y.cmp_gt(S::Vi64::zeroes()) & 1;
    let k3 = rank_z.cmp_gt(S::Vi64::zeroes()) & 1;
    let l3 = rank_w.cmp_gt(S::Vi64::zeroes()) & 1;

    let x1 = (x0 - i1.cast_f64()) + G4_64;
    let y1 = (y0 - j1.cast_f64()) + G4_64;
    let z1 = (z0 - k1.cast_f64()) + G4_64;
    let w1 = (w0 - l1.cast_f64()) + G4_64;
    let x2 = (x0 - i2.cast_f64()) + G24_64;
    let y2 = (y0 - j2.cast_f64()) + G24_64;
    let z2 = (z0 - k2.cast_f64()) + G24_64;
    let w2 = (w0 - l2.cast_f64()) + G24_64;
    let x3 = (x0 - i3.cast_f64()) + G34_64;
    let y3 = (y0 - j3.cast_f64()) + G34_64;
    let z3 = (z0 - k3.cast_f64()) + G34_64;
    let w3 = (w0 - l3.cast_f64()) + G34_64;
    let x4 = (x0 - 1.0) + G44_64;
    let y4 = (y0 - 1.0) + G44_64;
    let z4 = (z0 - 1.0) + G44_64;
    let w4 = (w0 - 1.0) + G44_64;
    ...
    let lp = S::i64gather_epi64(&PERM, ll);
    let kp = S::i64gather_epi64(&PERM, kk + lp);
    let jp = S::i64gather_epi64(&PERM, jj + kp);
    let gi0 = S::i64gather_epi64(&PERM, ii + jp);

    let lp = S::i64gather_epi64(&PERM, ll + l1);
    let kp = S::i64gather_epi64(&PERM, kk + k1 + lp);
    let jp = S::i64gather_epi64(&PERM, jj + j1 + kp);
    let gi1 = S::i64gather_epi64(&PERM, ii + i1 + jp);

    let lp = S::i64gather_epi64(&PERM, ll + l2);
    let kp = S::i64gather_epi64(&PERM, kk + k2 + lp);
    let jp = S::i64gather_epi64(&PERM, jj + j2 + kp);
    let gi2 = S::i64gather_epi64(&PERM, ii + i2 + jp);

    let lp = S::i64gather_epi64(&PERM, ll + l3);
    let kp = S::i64gather_epi64(&PERM, kk + k3 + lp);
    let jp = S::i64gather_epi64(&PERM, jj + j3 + kp);
    let gi3 = S::i64gather_epi64(&PERM, ii + i3 + jp);

    let lp = S::i64gather_epi64(&PERM, ll + 1);
    let kp = S::i64gather_epi64(&PERM, kk + 1 + lp);
    let jp = S::i64gather_epi64(&PERM, jj + 1 + kp);
    let gi4 = S::i64gather_epi64(&PERM, ii + 1 + jp);
    ...
    n0 = n0 & !t0.cmp_lt(S::Vf64::zeroes());
    n1 = n1 & !t1.cmp_lt(S::Vf64::zeroes());
    n2 = n2 & !t2.cmp_lt(S::Vf64::zeroes());
    n3 = n3 & !t3.cmp_lt(S::Vf64::zeroes());
    n4 = n4 & !t4.cmp_lt(S::Vf64::zeroes());

    // Scaling factor found by numerical approximation
    (n0 + n1 + n2 + n3 + n4) * 62.77772078955791
}
