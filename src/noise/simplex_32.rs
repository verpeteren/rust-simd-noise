...
pub unsafe fn simplex_2d_deriv<S: Simd>(
    ...
    let t = (i + j).cast_f32() * G2_32;

    // Unskewed distances to the first point of the enclosing simplex
    let x0 = x - (ips - t);
    let y0 = y - (jps - t);

    let i1 = x0.cmp_gte(y0).cast_i32();
    let j1 = y0.cmp_gt(x0).cast_i32();

    // Distances to the second and third points of the enclosing simplex
    let x1 = x0 + i1.cast_f32() + G2_32;
    let y1 = y0 + j1.cast_f32() + G2_32;
    let x2 = x0 + (-1.0) + G22_32;
    let y2 = y0 + (-1.0) + G22_32;
    ...
    let gi2 = S::i32gather_epi32(&PERM, (ii + 1) + S::i32gather_epi32(&PERM, jj + 1));

    // Weights associated with the gradients at each corner
    // These FMA operations are equivalent to: let t = 0.5 - x*x - y*y
    let mut t0 = y0.neg_mul_add(y0, x0.neg_mul_add(x0, S::Vf32::set1(0.5)));
    let mut t1 = y1.neg_mul_add(y1, x1.neg_mul_add(x1, S::Vf32::set1(0.5)));
    let mut t2 = y2.neg_mul_add(y2, x2.neg_mul_add(x2, S::Vf32::set1(0.5)));
...
pub unsafe fn simplex_3d_deriv<S: Simd>(
    ...
    let i = x0.cast_i32() * X_PRIME_32;
    let j = y0.cast_i32() * Y_PRIME_32;
    let k = z0.cast_i32() * Z_PRIME_32;

    // Compute distance from first simplex vertex to input coordinates
    let g = (x0 + y0 + z0) * G3_32;
    x0 = x - (x0 - g);
    y0 = y - (y0 - g);
    z0 = z - (z0 - g);
    ...
    let x1 = x0 - (i1 & 1.0) + G3_32;
    let y1 = y0 - (j1 & 1.0) + G3_32;
    let z1 = z0 - (k1 & 1.0) + G3_32;

    let x2 = x0 - (i2 & 1.0) + F3_32;
    let y2 = y0 - (j2 & 1.0) + F3_32;
    let z2 = z0 - (k2 & 1.0) + F3_32;
    ...
    let v1x = i + (i1.cast_i32() & X_PRIME_32);
    let v1y = j + (j1.cast_i32() & Y_PRIME_32);
    let v1z = k + (k1.cast_i32() & Z_PRIME_32);
    let g1 = grad3d_dot::<S>(seed, v1x, v1y, v1z, x1, y1, z1);
    let v1 = t41 * g1;

    let v2x = i + (i2.cast_i32() & X_PRIME_32);
    let v2y = j + (j2.cast_i32() & Y_PRIME_32);
    let v2z = k + (k2.cast_i32() & Z_PRIME_32);
    ...
    let v3x = i + X_PRIME_32;
    let v3y = j + Y_PRIME_32;
    let v3z = k + Z_PRIME_32;
...
pub unsafe fn simplex_4d<S: Simd>(
    ...
    let i1 = rank_x.cmp_gt(S::Vi32::set1(2)) & 1;
    let j1 = rank_y.cmp_gt(S::Vi32::set1(2)) & 1;
    let k1 = rank_z.cmp_gt(S::Vi32::set1(2)) & 1;
    let l1 = rank_w.cmp_gt(S::Vi32::set1(2)) & 1;

    let i2 = rank_x.cmp_gt(S::Vi32::set1(1)) & 1;
    let j2 = rank_y.cmp_gt(S::Vi32::set1(1)) & 1;
    let k2 = rank_z.cmp_gt(S::Vi32::set1(1)) & 1;
    let l2 = rank_w.cmp_gt(S::Vi32::set1(1)) & 1;

    let i3 = rank_x.cmp_gt(S::Vi32::zeroes()) & 1;
    let j3 = rank_y.cmp_gt(S::Vi32::zeroes()) & 1;
    let k3 = rank_z.cmp_gt(S::Vi32::zeroes()) & 1;
    let l3 = rank_w.cmp_gt(S::Vi32::zeroes()) & 1;

    let x1 = (x0 - i1.cast_f32()) + G4_32;
    let y1 = (y0 - j1.cast_f32()) + G4_32;
    let z1 = (z0 - k1.cast_f32()) + G4_32;
    let w1 = (w0 - l1.cast_f32()) + G4_32;
    let x2 = (x0 - i2.cast_f32()) + G24_32;
    let y2 = (y0 - j2.cast_f32()) + G24_32;
    let z2 = (z0 - k2.cast_f32()) + G24_32;
    let w2 = (w0 - l2.cast_f32()) + G24_32;
    let x3 = (x0 - i3.cast_f32()) + G34_32;
    let y3 = (y0 - j3.cast_f32()) + G34_32;
    let z3 = (z0 - k3.cast_f32()) + G34_32;
    let w3 = (w0 - l3.cast_f32()) + G34_32;
    let x4 = (x0 - 1.0) + G44_32;
    let y4 = (y0 - 1.0) + G44_32;
    let z4 = (z0 - 1.0) + G44_32;
    let w4 = (w0 - 1.0) + G44_32;
    ...
    let lp = S::i32gather_epi32(&PERM, ll + 1);
    let kp = S::i32gather_epi32(&PERM, kk + 1 + lp);
    let jp = S::i32gather_epi32(&PERM, jj + 1 + kp);
    let gi4 = S::i32gather_epi32(&PERM, ii + 1 + jp);
    ...
    n0 = n0 & !t0.cmp_lt(S::Vf32::zeroes());
    n1 = n1 & !t1.cmp_lt(S::Vf32::zeroes());
    n2 = n2 & !t2.cmp_lt(S::Vf32::zeroes());
    n3 = n3 & !t3.cmp_lt(S::Vf32::zeroes());
    n4 = n4 & !t4.cmp_lt(S::Vf32::zeroes());
