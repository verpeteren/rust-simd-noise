...
pub unsafe fn turbulence_2d<S: Simd>(
    ...
    for _ in 1..octaves {
        ...
        y = S::mul_pd(y, lac);
        amp = S::mul_pd(amp, gain);
        result = result + S::mul_pd(simplex_2d::<S>(x, y, seed), amp).abs();
    }
...
pub unsafe fn turbulence_3d<S: Simd>(
    ...
    for _ in 1..octaves {
        ...
        z = S::mul_pd(z, lac);
        amp = S::mul_pd(amp, gain);
        result = result + S::mul_pd(simplex_3d::<S>(x, y, z, seed), amp).abs();
    }
...
pub unsafe fn turbulence_4d<S: Simd>(
    ...
    for _ in 1..octaves {
        ...
        w = S::mul_pd(w, lac);
        amp = S::mul_pd(amp, gain);
        result = result + S::mul_pd(simplex_4d::<S>(x, y, z, w, seed), amp).abs();
    }
