    ...
    for _x in 0 .. 3 {
        ...
        for _y in 0 .. 3 {
            ...
            for _z in 0 .. 3 {
                ...
                let mut xd = S::sub_ps(S::cvtepi32_ps(S::and_epi32(hash,S::set1_epi32(BIT_10_MASK))),S::set1_ps(511.5));
                let mut yd = S::sub_ps(S::cvtepi32_ps(S::and_epi32(S::srai_epi32(hash,10),S::set1_epi32(BIT_10_MASK))),S::set1_ps(511.5));
                let mut zd = S::sub_ps(S::cvtepi32_ps(S::and_epi32(S::srai_epi32(hash,20),S::set1_epi32(BIT_10_MASK))),S::set1_ps(511.5));
                let inv_mag = S::mul_ps(jitter,S::rsqrt_ps(S::fmadd_ps(xd,xd,S::fmadd_ps(yd,yd,S::mul_ps(zd,zd)))));
                xd = S::fmadd_ps(xd,inv_mag,xcf);
                yd = S::fmadd_ps(yd,inv_mag,ycf);
